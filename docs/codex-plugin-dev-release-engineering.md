...
- `plugin-contract.yml`：Node 22/24、macOS/Linux gate；contract 预构建显式拆成 `core -> mcp-server -> discovery` 独立步骤，matrix 禁用 fail-fast，避免 clean checkout 被本地 workspace `dist` 掩盖或首个失败取消其余平台证据；Windows 只跑 version/contract/lint/typecheck/build 并明确 `/bin/sh` launcher 不支持 native Windows，不把全量 Windows runtime suite 混入该边界门禁。
