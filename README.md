...
- `pippit-video`：从 repo marketplace 安装的 Codex plugin，内嵌同一个 stdio MCP server。
...
| `PIPPIT_FACADE_TIMEOUT_MS` | `120000` | wrapper 调用 facade 的超时 |
...
Codex/MCP 的 completed 结果会先把完整 MP4 原子保存为 `PIPPIT_MCP_OUTPUT_ROOT` 下的普通本地文件，再返回 widget。默认目录在 macOS 是 `~/Movies/Pippit`，其他平台是 `~/Videos/Pippit`；不会写入 `/tmp`、仓库或 plugin cache。当前 stdio/plugin 进程持有一个同生命周期的 loopback HTTP server，只从这份本地文件处理 HEAD/Range，并补齐 CORS 与 Private Network Access 响应；它不会把远程签名地址交给播放器。stdin EOF 或 plugin disable 后端口关闭，但 MP4 保留。`pippit_download_video` 只用于创建另一个用户指定文件名或路径的副本。
