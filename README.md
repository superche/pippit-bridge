...
从公开 npm registry 全局安装 OpenCode custom-tool plugin：
...
- OpenRouter 风格的图片/视频生成 facade 与服务器持久化 BYOK。
- 可发布的 Pippit TypeScript SDK、共享模型目录与安全素材能力。
- `@pippit-bridge/opencode-plugin`：不注册 LLM provider/auth slot 的 OpenCode 本地视频 custom-tool plugin。
...
**架构定位：这是面向单个本地用户的 plugin bridge，不是多租户 SaaS。** 主要产品面是 Codex `pippit-video` plugin 与 `@pippit-bridge/opencode-plugin`；通用 MCP、loopback Facade 和本地 ChatGPT developer app 服务于同一用户、同一台受信主机上的接入与调试。项目不以多用户 OAuth、租户隔离、分布式锁、横向扩容或跨机器状态同步为当前目标。

**当前三种封装提供图片与视频生成。** 图片由 Seedream 5.0 / 5.0 Pro 同步返回，视频仍使用异步 job、轮询与本地结果 widget。图片、视频和音频也可作为视频参考素材；这不代表已提供通用文本、语音生成或转录工具。通用 MCP 与 Codex plugin 另外提供 facade 账号管理；ChatGPT App 的 `noauth` developer-mode surface 不暴露这组管理工具。
...
`core` 是模型版本与安全素材真源；`sdk` 只封装小云雀官方 AK API；OpenCode plugin 复用 MCP package 的一次性 loopback enrollment，并直接使用 core + sdk 完成视频任务。MCP server 只调用 facade，ChatGPT App 再复用 MCP 的工具实现；Codex plugin 的 manifest、skill 和 `.mcp.json` 直接随 `packages/mcp-server-pippit` 分发。后续的 CLI、ComfyUI、n8n 和 OpenMontage adapter 仍可作为新 workspace 增加，不需要复制上游协议实现。
...
| ChatGPT App | `https://<host>/mcp` | 投影 MCP 的图片生成，以及视频列模型、生成、查询和参考视频重新生成；视频结果提供 widget | ChatGPT developer mode；本地或 tunnel 调试 |
...
`pippit_generate_video` 与 `pippit_edit_video_segment` 的 `idempotency_key` 是可选异常恢复键，不属于 Facade/OpenRouter 协议。图片接口按 OpenRouter dedicated Image API 同步返回，不暴露这一 MCP 恢复字段。完整边界见 [持久化幂等设计](./docs/idempotency.md)。

## OpenCode custom-tool plugin

OpenCode 1.18.3 的 model provider contract 是 AI SDK `LanguageModelV3`。小云雀当前公开的是异步视频任务 API，不是聊天/流式语言模型；本项目不会把视频模型伪装成聊天模型。这个包只注册 custom tools，不返回 `auth`/`provider` hook，也不写入 `config.provider.pippit`，因此不会过滤或覆盖宿主默认 LLM：

- `pippit_manage_access_keys`：返回一次性 localhost password form，并脱敏列出、切换和删除多个本地账号的 AK。
- `pippit_generate_video`：上传参考素材并提交视频生成；每次付费提交前显式请求权限。
- `pippit_generate_image`：使用 Seedream 5.0 / 5.0 Pro 生成图片，可带最多 9 张参考图；Codex/stdio 先将完成图片保存到本地 output root，再通过稳定 artifact 地址预览并提供原图下载。
...
让 OpenCode 配置一个账号时，调用 `pippit_manage_access_keys` 的 `configure` 操作并传入本地账号名。工具返回短时、单次使用的 `http://127.0.0.1:<port>/enroll/<token>`；用户在浏览器 password form 中粘贴 AK，表单直接写入插件私有 keyring 并设为 active。raw AK 不进入聊天、普通工具参数、OpenCode auth slot、URL query 或日志。
...
多账号秘密保存在 OpenCode 全局 state 下的 `pippit/access-keys.json`，而不是项目目录或 OpenCode provider auth store。父目录使用 `0700`、文件使用 `0600`，原子替换写入；当前内容是同 UID 可读的明文，不是系统 keychain。plugin 不读取 Cookie、不监听剪贴板、不把 AK 放进普通 tool 参数、项目配置、日志或 telemetry。
...
facade 底层调用小云雀 Seedream 图片生成与沉浸式短片视频 API，并提供服务器持久化 BYOK（Bring Your Own Key）。
...
对图片、视频和音频参考素材，服务会先下载每个 URL，逐个调用小云雀上传接口取得 `data.pippit_asset_id`，全部成功后再根据目标提交图片或视频任务：
...
| Facade API Key | 部署方生成并发给调用方；服务配置中只保存 SHA-256 allowlist | `/api/v1/models`、`/api/v1/images**`、`/api/v1/videos/**` |
...
当前 facade 只解析静态 Facade API Key，没有 per-user identity。`allowed_user_ids` 只有为 `null` 时才能用于当前运行时路由；一旦写成非空列表，任何图片或视频生成请求都不会匹配该 credential。保留这个字段是为了契约兼容和未来扩展，不代表已经支持 user routing。
...
图片只暴露 `pippit/seedream-5.0` 与 `pippit/seedream-5.0-pro`，省略 `model` 时默认使用 Seedream 5.0。`pippit/seedream-5.0` 必须完全省略 `resolution`；`pippit/seedream-5.0-pro` 可省略或使用 `1K`、`2K`、`4K`。`input_references` 最多 9 张，图片完成后以 OpenRouter dedicated Image API 的 `data[].b64_json` 同步返回。
...
curl -X POST http://localhost:3000/api/v1/images \
  ...
  -d '{
    ...
    "prompt": "未来感运动鞋广告海报，黑色背景，蓝紫霓虹边缘光",
    "resolution": "4K",
    "n": 1
  }'
```
...
| Facade model | Pippit model | 通道 | 分辨率 |
| --- | --- | --- | --- |
| `pippit/seedance-2.0-mini`（默认） | `Seedance_2.0_mini` | VIP | `480p`, `720p` |
| `pippit/seedance-2.0` | `seedance2.0_direct` | 非 VIP | `480p`, `720p` |
| `pippit/seedance-2.0-mini-lite` | `Seedance_2.0_mini_lite` | 非 VIP | `480p`, `720p` |
| `pippit/seedance-2.0-vision` | `seedance2.0_vision` | VIP | `480p`, `720p`, `1080p` |

公开生成接口只接受表格中的稳定 facade id；原始 Pippit model 字符串仅用于内部上游映射，不作为可调用模型暴露。
...
| `IMAGE_GENERATION_POLL_INTERVAL_MS` | `3000` | 同步图片接口查询生成状态的轮询间隔（3 秒） |
| `IMAGE_GENERATION_TIMEOUT_MS` | `43200000` | 同步图片接口从提交到完成的总等待上限（12 小时） |
...
| `PIPPIT_MCP_OUTPUT_ROOT` | macOS `~/Movies/Pippit`；其他平台 `~/Videos/Pippit` | stdio MCP / Codex 的 completed 图片、MP4 与额外视频副本 root；可覆盖 |
...
Codex/MCP 的 completed 图片和视频会先原子保存为 `PIPPIT_MCP_OUTPUT_ROOT` 下的普通本地文件，再返回 widget。默认目录在 macOS 是 `~/Movies/Pippit`，其他平台是 `~/Videos/Pippit`；不会写入 `/tmp`、仓库或 plugin cache。图片 widget 读取完整本地图片，视频 widget 分块读取本地 MP4，并分别创建沙箱内 `blob:` 预览地址；不会把远程签名地址、`file://` 路径或普通 HTTP loopback 地址交给预览器。stdio 重启后仍可按稳定 artifact identity 重新读取，本地文件继续保留。`pippit_download_video` 只用于创建另一个用户指定文件名或路径的视频副本。
