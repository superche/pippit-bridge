...
`pippit_generate_video`, `pippit_get_video`, and `pippit_edit_video_segment` advertise one shared MCP App resource. After polling reaches `completed`, a supporting host such as Codex renders the inline preview and segment editor from the `pippit_get_video` result automatically. Before returning that result, the stdio/plugin process fully downloads and atomically publishes an ordinary MP4 under its output root. The same process then serves only that local file through a loopback HTTP listener with HEAD, Range, CORS, and Private Network Access support. The listener lives for the stdio/plugin process and closes at EOF; the MP4 remains on disk. Upstream content URLs and `unsigned_urls` never reach the player or model-visible output. `pippit_download_video` is only needed for an additional user-chosen file name or destination.
...
The repo-local marketplace is for development. Run `npm run build -w @pippit-bridge/mcp-server` before installing it from a clean checkout. After publishing version `0.2.6`, use the npm marketplace shape in `.agents/plugins/marketplace.npm.example.json` for an end-user install.
