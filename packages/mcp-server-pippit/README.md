...
The repo-local marketplace is for development. Run `npm run build -w @pippit-bridge/mcp-server` before installing it from a clean checkout. After publishing version `0.2.12`, use the npm marketplace shape in `.agents/plugins/marketplace.npm.example.json` for an end-user install.
