...
3. For images, call `pippit_list_image_models` when needed, then call `pippit_generate_image`. Use `pippit/seedream-5.0` without `resolution`, or `pippit/seedream-5.0-pro` with optional `1K`, `2K`, or `4K`. Up to 9 reference images and `n=1..10` are accepted. In Codex/stdio, completed bytes are atomically saved locally before the result card opens; the card previews that stable local artifact and provides `Download original`. There is no image polling or separate model-visible download tool. If the user asks for the same image or file again, direct them to the existing card or configured output folder and never regenerate it.
