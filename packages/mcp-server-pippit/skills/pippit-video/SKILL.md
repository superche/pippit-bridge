...
4. For video, call `pippit_list_video_models` when the model or its supported settings are not already known. Omit `model` to use the officially released default `pippit/seedance-2.5`; actual submission remains subject to the active Pippit account's upstream entitlement. The production-aligned alternatives are `pippit/seedance-2.0-vision`, `pippit/seedance-2.0-fast`, `pippit/seedance-2.0-mini`, and `pippit/seedance-2.0-mini-lite`.
...
Use `pippit_edit_video_segment` only with a completed source job and a `time_range` within the source video. The result widget and direct calls default to `pippit/seedance-2.0-vision` (`seedance2.0_vision` upstream); a supported Seedance 2.0-family source model is preserved. Seedance 2.0-family edits use the complete source as an ordinary video reference with a positive output duration, while an explicitly selected `pippit/seedance-2.5` uses native `partial_edit_videos`. Seedance 2.0-family ranges must be 4–15 seconds and Seedance 2.5 ranges must be 4–30.2 seconds. Public MCP and ChatGPT arguments use microseconds. For Seedance 2.0-family reference-guided regeneration, `time_range`, optional `guidance_annotations[].at_time_us`, normalized intrinsic-frame `region`, and `instruction` are prompt guidance rather than a native provider range or hard mask.

The upstream contracts do not expose a pixel mask. Describe the result as regeneration, not as an in-place byte edit or proof that an exact rectangle mask was enforced.
