...
4. For video, call `pippit_list_video_models` when the model or its supported settings are not already known. Omit `model` to use the default VIP-channel `pippit/seedance-2.0-mini`. The only other video models are non-VIP `pippit/seedance-2.0` / `pippit/seedance-2.0-mini-lite` and VIP `pippit/seedance-2.0-vision`.
