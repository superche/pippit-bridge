...
For a completed result, the widget supports Seedance video regeneration modeled after the supplied “片段重拍” interaction: choose a range within the source video, seek and drag a rectangle over intrinsic video content, and enter the requested visible change. Editing defaults to `pippit/seedance-2.0-vision` (`seedance2.0_vision` upstream); a supported Seedance 2.0-family source model is preserved. Seedance 2.0-family edits attach the complete source as an ordinary video reference with a positive duration, so the selected range and normalized rectangles are prompt guidance rather than native provider masks. An explicitly selected `pippit/seedance-2.5` continues to use native `partial_edit_videos`. Seedance 2.0-family ranges are 4–15 seconds and Seedance 2.5 ranges are 4–30.2 seconds. Signed preview URLs, asset identities, and local absolute paths are never copied into public `structuredContent`.
