...
- Contract goldens are generated from a clean build of the real launcher and discovery surface. The npm commands build `@pippit-bridge/core` before the MCP package so clean worktrees cannot reuse stale workspace output; run `npm run generate:plugin-contract` only for an intentional cold-contract release, review the diff, and keep `npm run check:plugin-contract` green.
